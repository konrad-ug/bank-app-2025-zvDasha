...
@pytest.fixture
def sut():
    return PersonalAccount("John", "Doe", "01345678910")

def test_account_creation(sut):
    assert sut.first_name == "John"
    assert sut.last_name == "Doe"
    assert sut.balance == 0 
    assert sut.pesel == "01345678910"
    assert sut.get_birth_year_from_pesel() == 2001
...
def test_pesel_invalid(invalid_pesel):
...
@pytest.mark.parametrize("pesel, promo_code, expected_balance", [
    ...
    ("61010112345", "AROM_132", 0.0),       
    ("61010112345", "prom_123", 0.0),    
    ("61010112345", "PROM_12", 0.0),     
    ("61010112345", "PROM_1234", 0.0),  
    ("61010112345", None, 0.0),   
    ("61010112345", "PROM_123", 50.0), 
    ("50010112345", "PROM_123", 0.0),    
    ("123", "PROM_123", 0.0),       
])
def test_promo_code(pesel, promo_code, expected_balance):
    account = PersonalAccount("Jane", "Doe", pesel, promotion_code=promo_code)
